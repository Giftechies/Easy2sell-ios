...
export default function EditItem({navigation,route}) {
  ...
  const getPostToEdit = async (params) => {
    try {
      ...
      if(response.success){
        ...
        setQty(response.data.qty.toString());
        setDescription(response.data.description);
        setCategory(response.data.category.name);
        setTags({tag:'',tagsArray:response.data.tagsArray})
        setSelectedCategory(response.data.category);
        setSelectedCondition(response.data.condition);
        setCondition(response.data.condition.name);
  ...
  useEffect(() => {  
    ...
    setCondition(selectedCondition.name);
    }
  },[selectedCondition]);

  const onSelect = (category,title=null) => {
    if(title!='select_condition'){
      setSelectedCategory(category);
    }else{
      setSelectedCondition(category);
    }
  };
  ...
  const createPost = async () => {
    if (title == '' || images.length==0 && oldImages.length==0  || location == '' || category == '' || condition == '' || price == '' || qty == '') {
        setSuccess({
            ...success,
      ...
      formdata.append('condition',selectedCondition.id);
      formdata.append('description',description);
      try {
          const response = await api.editItem(formdata,{
          ...
          <ScrollView>
              ...
              <View style={{flexDirection:'row',flexWrap:'wrap'}}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    navigation.navigate('CommonSelectModelB',{categories:conditions,title:'select_condition',onSelect:onSelect});
                  }}>
                  <Text style={{color:!success.condition?colors.primary:colors.gray}}>{t('condition')}</Text>
                  <View
                    style={{
                      flexDirection: 'row',
                      alignItems: 'center',
                ...
                <TagInput
                  updateState={tags =>{console.log(tags); setTags(tags)}}
                  tags={tags}
                  placeholder="Tags..."                            
