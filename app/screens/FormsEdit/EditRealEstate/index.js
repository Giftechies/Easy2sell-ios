...
export default function EditRealEstate({navigation,route}) {
  ...
  const getPostToEdit = async (params) => {
    try {
      ...
      if(response.success){
        ...
        setBuiltupArea(response.data.builtup_area);
        setCarpetArea(response.data.carpet_area);
        
        setPropertyType(response.data.property_type.name);
        setSelectedPropertyType(response.data.property_type);
        setAvailableForType(response.data.available_for.name);
        setSelectedAvailableForType(response.data.available_for);
        setSelectedAmenityType(response.data.home_amenities);
  ...
  const createPost = async () => {
    if (title == '' || images.length==0 && oldImages.length==0 || bedrooms == '' || bathrooms == '' || location == '' || description == '' || propertyType == '' || availableForType == '') {
        setSuccess({
            ...success,
      ...
      try {
        ...
        const response = await api.editRealEstate(formdata,{
