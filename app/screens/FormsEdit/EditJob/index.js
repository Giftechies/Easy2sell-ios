...
export default function EditJob({navigation,route}) {
  ...
  const getPostToEdit = async (params) => {
    try {
      ...
      if(response.success){
        ...
        setMaximumSalary(response.data.maximum_salary);
        setLocation(response.data.location);
        setLocationData(response.data.locationdata);
        setSelectedSalaryType(response.data.salary_type);
        setSalaryType(response.data.salary_type.name);
        setSelectedJobType(response.data.job_type);
        setJobType(response.data.job_type.name);
  ...
  const createPost = async () => {
    if (title == '' || businessName == '' || images.length==0 && oldImages.length==0 || category == '' || salaryType == '' || minimumSalary == '' || maximumSalary == '' || location == '' || jobType == '' || description == '') {
        setSuccess({
            ...success,
      ...
      try {
          const response = await api.editJob(formdata,{
