...
export default function AddCoupon({navigation}) {
  ...
  const createPost = async () => {
    if (title == '' || category == '' || brandLogo.length==0 || code == '' && type=='coupon' || type == '' || referralUrl == '' || startDate == '' || expiryDate == '' || description == '') {
        setSuccess({
            ...success,
            title: title != '' ? true : false,
            brandLogo: brandLogo.length > 0 ? true : false,
      ...
      try {
          const response = await api.saveCoupon(formdata,{
          ...
          if(response.success){
            ...
            Alert.alert({
                ...
                action: [{ onPress: () => { 
                    //navigation.goBack() 
                    //dispatch({type:actionTypes.LOGIN_SUCCESS,user:response.user})
