...
export default function CommonMultiSelectModelB({ route,navigation }) {
  ...
  const onChange = (select) => {
    setCategorySelected((prevState) =>  {
        ...
        if(prevState.some((cat) => cat === select)){
            prevState = prevState.filter(function(c) { 
                return c !== select
            })
            return prevState
        }
        return [...prevState, ...[categories[categories.indexOf(select)]]]
    });
  };
        ...
        <View style={styles.contain}>
          ...
          <FlatList
            ...
            renderItem={({ item,index }) => {
              ...
              const selected = categorySelected.some((cat) => cat === item);
